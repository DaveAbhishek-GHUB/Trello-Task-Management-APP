...
function HomePage() {
  ...
  const onSubmitList = (ListData) => {
    setIsAddList(false);
    dispatch(CreateList({ boardID: selectedBoardId, listname: ListData.listname }));
  };
  ...
  const onDragEnd = (result) => {
    ...
    const card = sourceList.cards.find((card) => card.id === parseInt(draggableId));
    ...
    <div className="main-homepage-wrapper w-full min-h-screen flex bg-blue-50 relative">
      {isCardEdit && (
        <div className="Edit-section-wrapper w-[80vw] h-[40vw] bg-white absolute top-[6vw] left-[18vw] rounded-xl border-2 max-md:w-[90vw] max-md:left-[5vw] max-sm:w-full max-sm:left-0">
          <div className="close-btn-wrapper w-full flex justify-end border-b-2">
            <button onClick={() => setIsCardEdit(false)} className="p-5 rounded-full">
              <CloseBTN />
            </button> 
          </div>
          ...
          <div className="card-info-wrapper w-full flex gap-5 p-5">
            ...
            <span className="text-[2vw] max-md:text-[3vw] max-sm:text-[4vw]">{editCard.name}</span>
          </div>
          <div className="card-description-wrapper flex justify-center items-center px-3">
            {isEditingDescription ? (
              <form className="w-full flex justify-center mt-2" onSubmit={handleSubmit(onSubmitDescription)}>
                <div className="Forcarddescription m-auto w-full px-3">
                  <input
                    ...
                    {...register("carddescription", {
                      ...
                      validate: (value) => value.trim().length > 0 || "Card description cannot be just whitespace",
                    })}
                  />
      ...
      {boardId !== null ? (
                  ...
                  <div key={list.id} className="inner-list-wrapper">
                    <div className="list w-[18vw] bg-blue-800 rounded-xl shadow-lg max-md:w-full">
                      <div className="list-heading-wrapper flex items-center justify-between text-white text-[1.5vw] max-md:text-[2.5vw] max-sm:text-[3.5vw] px-4 py-3 border-b border-blue-700">
                        <span className="font-semibold truncate text-[1vw] max-md:text-[2vw] max-sm:text-[2.5vw]">{list.name}</span>
                        <button
                          onClick={() => {
                            setListId(list.id);
                            setIsAddCard(true);
                        ...
                        {(provided) => (
                          ...
                          >
                            ...
                            {isAddCard && listId === list.id && (
                              <form className="w-full flex gap-2 items-start mt-2" onSubmit={handleSubmit(onSubmitCard)}>
                                <div className="flex-1">
                                  <input
                                    className="w-full px-3 py-2 text-[1.5vw] max-md:text-[2.5vw] max-sm:text-[3.5vw] border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    ...
                                    {...register("cardname", {
                                      ...
                                      validate: (value) => value.trim().length > 0 || "Card name cannot be just whitespace",
                                    })}
                                  />
                                  {errors.cardname && (
                                ...
                                <button
                                  type="submit"
                                  className="text-[1.5vw] max-md:text-[2.5vw] max-sm:text-[3.5vw] bg-blue-800 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors whitespace-nowrap"
                                >
                                  Add
                                </button>
                              </form>
        ...
        <div className="main-wrapper w-full text-[3vw] font-sans flex justify-center items-center">
          <span className="text-[3vw] max-md:text-[4vw] max-sm:text-[5vw]">SELECT YOUR BOARD</span>
        </div>
      )}
    </div>
  ) : (
